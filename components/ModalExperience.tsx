...
import { FiX, FiCheck, FiPlus, FiSearch } from 'react-icons/fi';
import { supabase } from '@/lib/supabase';
import { Experience, ExperienceFormData, SkillWithLevel } from '@/context/ModalContext';
...
export default function ExperienceModal() {
  ...
  // Verificar si estamos en modo creación/edición
  if (mode === 'view') return null;

  const formData = experienceData as ExperienceFormData;
  ...
  if (
    skill.trim() &&
    !experienceData.skills.some(s => s.name === skill.trim())
  ) {
    // Buscar la habilidad en el array de habilidades para obtener su ID
    const habilidad = habilidades.find(h => h.Nombre === skill.trim());
    if (!habilidad) return; // No agregar si no se encuentra
...
const updateSkillLevel = (skillName: string, newLevel: 'beginner' | 'intermediate' | 'expert') => {
  ...
  const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setExperienceData(prev => ({
      ...prev,
      [name]: checked,
      ...(name === 'currentJob' && checked ? { endDate: '' } : {})
    }));
  };
  ...
  const experienceId = crypto.randomUUID(); // Genera el UUID

  try {
    // 1. Guardar la experiencia en la tabla Historial
    ...
    // 2. Guardar cada habilidad asociada en la tabla correspondiente
    // (asumo que se llama 'Experiencia_Habilidad' o similar)
    if (experienceData.skills.length > 0) {
      const habilidadesInsert = experienceData.skills.map(skill => {
        // Encontrar la habilidad en el listado para obtener su ID
        const habilidad = habilidades.find(h => h.Nombre === skill.name);
        if (!habilidad) {
          console.warn(`No se encontró la habilidad: ${skill.name}`);
          return null;
        }

        return {
          ID_Historial: experienceId,
          ID_Habilidad: habilidad.ID_Habilidad,
          nivel: skill.level,
          // Puedes agregar más campos si es necesario
        };
      }).filter(Boolean); // Filtra cualquier null por si hay habilidades no encontradas

      if (habilidadesInsert.length > 0) {
        const { error: habilidadesError } = await supabase
          .from('Historial_Habilidades') // Ajusta el nombre de la tabla según tu esquema
    ...
    console.log('Experiencia y habilidades guardadas correctamente');
    closeModal();
    resetModal();
    refreshSkills();
  } catch (err) {
    console.error('Error al guardar la experiencia:', err);
    setError('Error al guardar la experiencia. Por favor intenta nuevamente.');
  } finally {
    setIsSaving(false);
  }
};
      ...
      <div className="relative bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto transform transition-all">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center z-10">
          <h3 className="text-xl font-semibold text-gray-900">
            {experienceData.position ? 'Editar experiencia' : 'Añadir experiencia'}
          </h3>
          <button
            onClick={() => {
              closeModal();
...
<div className="mt-4">
  <h4 className="text-sm font-medium text-gray-700 mb-2">Habilidades y Herramientas seleccionadas:</h4>
    ...
    <p className="text-sm text-gray-500">No has seleccionado ninguna habilidad</p>
        ...
        <div className="sticky bottom-0 bg-gray-50 px-6 py-4 border-t border-gray-200 flex justify-end space-x-3">
          <button
            ...
            className="px-4 py-2 border border-purple-300 rounded-md text-gray-700 hover:bg-gray-100 transition-colors"
          >
            Cancelar
          </button>
          <button
