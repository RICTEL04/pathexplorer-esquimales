...
test.describe('Browser Suite', () => {
  test.describe('HU_06', () => {
    test('ID: TC-HU06.CA Creacion de Experiencia Laboral sin fecha final', async ({ page, browserName }, testInfo) => {
      // Configurar metadatos para Qase
      configureQaseTest({
        hu: 'HU_06',
        title: 'ID: TC-HU06.CA Creacion de Experiencia Laboral sin fecha final',
        description: `**Descripción**: El empleado puede crear una experiencia laboral sin la fecha final.
...
**Navegador**: ${browserName}`,
      ...
      const nombreExperiencia = 'QA sin fecha' + browserName; 
      try {
        ...
        await test.step('Crear experiencia laboral sin fecha final', async () => {
          ...
          await page.getByRole('checkbox', { name: 'Trabajo actual' }).check();
      ...
      await test.step('Buscar Experiencia y borrarla', async () => {
        ...
        while (!found && currentPage <= maxPages) {
          ...
          if (await experiencia.count() > 0) {
            ...
            // Espera a que ya no sea visible
