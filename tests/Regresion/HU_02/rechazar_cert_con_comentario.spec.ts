...
test.describe('Browser Suite', () => {
  test.describe('HU_02', () => {
    test('ID: TC-HU2.CA3-N - Rechazo de certificación con comentario Regresion', async ({ page, browserName }, testInfo) => {
      // Configurar metadatos para Qase
      configureQaseTest({
        hu: 'HU_02',
        title: 'ID: TC-HU2.CA3-N - Rechazo de certificación con comentario Regresion',
        description: `**Descripción**: El people lead quiere ver si se puede rechazar la certificacion
...
**Navegador**: ${browserName}`,
      ...
      const nombreTitulo = "certificado " + browserName
      ...
      try {
        ...
        await test.step('Rechazar certificado con comentario', async () => {
          ...
          // 4. Haz fill en el textarea del formulario
          const textarea = page.locator('tr.dropdown-row textarea.form-input');
          await expect(textarea).toBeVisible();
          await textarea.fill('Certificado rechazado por pruebas automatizadas');
